...
function buildPrompt({ text, mode, subStyle, targetLanguage, rephrase, injections }) {
  ...
  // ✅ Advanced QA block (kept from your feature)
  const QA_BLOCK = `
...
`;
  ...
  const renderedQA = safeRenderTemplate(QA_BLOCK, {
    TARGET_LANG: targetLanguage || 'the same language as the input',
    TEXT: text,
  });
...
DO NOT translate; keep the original language. Rephrase to match the requested style and sub-style. Maintain meaning and improve clarity. Return ONLY the final output strictly between <result> and </result>. No explanations.
${commonTail}`.trim();
  }
